// tpra wade L3K herringbone gears, based on :
...
printing = 0;
/* 0 = gears together
*  1 = both gears oriented for printing
*  2 = only big gear
*  3 = only small gear
*/
//distance_between_axles = 39.4555; // Distance between the midpoints of the two gears
distance_between_axles = 41.5; // Distance between the midpoints of the two gears
gear_height = 11; // Height of the actual gear teeth
teeth_twist = 1; // 1 means each tooth will slant up one additional tooth
...
/* Shaft diameter - Be sure to add some tolerance for printer error */
gear1_shaft_diameter = 5 + 0.3;

/* Set-screw diameter, mind tolerance */
gear1_setscrew_diameter = 3 + 0.4;

/* Set-nut width, measured across the flat sides, mind tolerance */
gear1_setnut_width = 5.5 + 0.3;

/* Set-nut height, mind tolerance */
gear1_setnut_height = 2.3 + 0.2;

/**************************\
 * Big gear configuration *
\**************************/

gear2_teeth = 47; // Number of teeth
gear2_outer_thickness = 5; // Extra size of the outer rim
gear2_shaft_diameter = 8 + 0.4; // Shaft diameter + tolerance
gear2_shaft_height = 10.5;
gear2_middle_diameter = 16;
gear2_middle_rounding = 0;
gear2_nut_diameter = 13 + 0.4; // nut width, measured across the flat sides + tolerance
gear2_nut_sunk = 6; // depth that the nut should be sunk inside the holder
...
gear2_decoration_flower = 6; //Set to number of petals (7 is nice)
...
module gear2() {
    ...
    difference() {
        union() {
            ...
            cylinder(h=gear2_shaft_height + gear2_nut_sunk -gear2_middle_rounding + 1, r=gear2_shaft_diameter/2 +1.5);
        ...
        translate([0,0,-epsilon+gear2_nut_sunk+0.6])
            hole(h=gear2_shaft_height + gear2_nut_sunk + 2*epsilon +2, r=gear2_shaft_diameter/2, $fn=24);
        //translate([0,0,gear2_shaft_height])
        rotate([0,0,30])
            translate([0,0,-1])
                hole(h=gear2_nut_sunk + epsilon+1, r=gear2_nut_diameter/2, $fn=6);
    }
}
        ...
        for (r=[0:360/number:360])
            rotate([0,0,r])
                translate([0,(inner_radius + outer_radius)/2,-epsilon])
                    cylinder(h=height+2*epsilon, r=size/2.2);
    }
}
...
module gear2_decorate_flower(inner_radius, outer_radius, number) {
    ...
    spacing = 1;
    size = (outer_radius - inner_radius) - 2*spacing - 3.5;
    ...
    difference() {
        ...
        for (r=[0:360/number:360])
            rotate([0,0,r])
                translate([inner_radius + spacing + radius +1,0,height/2])
                    rotate([0,90,0])
...
if (printing == 0) {
    ...
    translate([distance_between_axles/2 + 5,0,gear1_base_height])
        rotate([0,0,180])
            gear1();
